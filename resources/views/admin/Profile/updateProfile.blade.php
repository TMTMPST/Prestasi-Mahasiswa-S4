...
<div class="container py-4">
    <div class="card dashboard-card mb-4">
        <div class="card-header">
            <strong>Update Profil Admin</strong>
        </div>
        <div class="card-body">
            <form action="{{ route('admin.profile.update', $admin->username) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-3">
                    <label for="nip" class="form-label">Username</label>
                    <input type="text" class="form-control" id="nip" name="username" value="{{ old('username', $admin->username) }}" readonly>
                </div>
                ...
                <a href="{{ route('admin.profile.index') }}" class="btn btn-outline-maroon">Batal</a>
                @if ($errors->any())
